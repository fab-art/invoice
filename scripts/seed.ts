...
async function main() {
  ...
  const { error: adminError } = await supabase
    .from('User')
    .upsert(
      { email: 'admin@rssb.local', password: adminPassword, fullName: 'RSSB Administrator', role: 'ADMIN' },
  ...
  const { error: receptionError } = await supabase
    .from('User')
    .upsert(
      { email: 'reception@rssb.local', password: receptionPassword, fullName: 'Marie Receptionist', role: 'RECEPTIONIST' },
      { onConflict: 'email', ignoreDuplicates: true }
    );
  ...
  for (const p of SEED_PHARMACIES) {
    const { error } = await supabase
      .from('Pharmacy')
      .upsert(
        { pharmacyCode: p.pharmacy_code, pharmacyName: p.pharmacy_name, district: p.district },
        { onConflict: 'pharmacyCode', ignoreDuplicates: true }
      );
