...
import { SEED_PHARMACIES, buildYearPeriods } from './constants'
...
export async function ensureSeeded() {
  ...
  if (periodCount === 0) {
    ...
    for (const period of buildYearPeriods(year)) {
      await db.put('periods', { id: uuid(), ...period })
    }
  }
  const pharmacyCount = await db.count('pharmacies')
  if (pharmacyCount === 0) {
    for (const p of SEED_PHARMACIES) {
      await db.put('pharmacies', { id: uuid(), active: true, ...p })
    }
  }
...
export async function setActivePeriod(periodId) {
  ...
  for (const p of all) {
    await tx.store.put({ ...p, is_active: p.id === periodId })
  }
  await tx.done
}
